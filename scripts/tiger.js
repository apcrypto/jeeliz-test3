...
function generateSprite() {
  // generate a canvas2D used as texture for particle sprite material:
...
function initParticle(particle, delay, direction) {
  // init 1 particle position and movement
  ...
  new TWEEN.Tween(particle.position)
    .to(
      { x: direction.x * 10, y: direction.y * 10, z: direction.z * 10 },
      delay
    )
    .start()
    .onComplete(function () {
    ...
    uniforms: Object.assign(
      {
        ...
        resolution: {
          value: new THREE.Vector2(
            THREESTUFF.renderer.getSize().width,
            THREESTUFF.renderer.getSize().height
          ),
        },
...
function init_threeScene(spec) {
  ...
  for (let i = 0; i <= 200; ++i) {
    // we work with a fixed number of particle to avoir memory dynamic allowation
      ...
      if (ISDETECTED) {
        ...
        if (mouthOpening > 0.5) {
          ...
          PARTICLEDIR.set(
            ...
            0.5 * Math.sin(theta),
            1
          ).applyEuler(THREESTUFF.faceObject.rotation);
          initParticle(
            PARTICLES[PARTICLESHOTINDEX],
            2000 + 40 * Math.random(),
            PARTICLEDIR
          );
